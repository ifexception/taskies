...
int TaskRepository::GetByDateRange(std::vector<std::string> dates, std::vector<TaskRepositoryModel> models)
{
    for (const auto& date : dates) {
        ...
        int rc = sqlite3_prepare_v2(pDb,
            TaskRepository::getByDateRange.c_str(),
            static_cast<int>(TaskRepository::getByDateRange.size()),
            &stmt,
            nullptr);
        if (rc != SQLITE_OK) {
            const char* err = sqlite3_errmsg(pDb);
            pLogger->error(
                LogMessage::PrepareStatementTemplate, "TaskRepository", TaskRepository::getByDateRange, rc, err);
        ...
        if (rc != SQLITE_ROW) {
            const char* err = sqlite3_errmsg(pDb);
            pLogger->error(LogMessage::ExecStepTemplate, "TaskRepository", TaskRepository::getByDateRange, rc, err);
            sqlite3_finalize(stmt);
            return -1;
        }
        ...
        model.ProjectName = std::string(reinterpret_cast<const char*>(res), sqlite3_column_bytes(stmt, columnIndex++));
        res = sqlite3_column_text(stmt, columnIndex);
        model.CategoryName = std::string(reinterpret_cast<const char*>(res), sqlite3_column_bytes(stmt, columnIndex++));
...
const std::string TaskRepository::getByDateRange = "SELECT "
                                                   "tasks.task_id, "
                                                   "tasks.billable, "
